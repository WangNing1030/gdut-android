...
public class MusicService extends Service {
    ...
    public boolean onUnbind(Intent intent) {
        Log.i(TAG, "onUnBind: 服务解除绑定");
        return super.onUnbind(intent);
    }

    @Override
    public IBinder onBind(Intent intent) {
        Log.i(TAG, "onBind: 服务绑定启动");
    ...
    class MusicControl extends Binder {
        public void play(MusicBeen music) {
            try {
                player.reset();//重置音乐播放器
                //加载多媒体文件
                player = MediaPlayer.create(getApplicationContext(), music.getMusicId());
                player.start();//播放音乐
                addTimer();     //添加计时器
            } catch (Exception e) {
                e.printStackTrace();
            }
            // handler 到 MainActivity 更新 title， image
            new Thread(() -> {
                Message message = MainActivity.handler.obtainMessage();
                Bundle bundle = new Bundle();
                bundle.putString("musicTitle", music.getTitle());
                bundle.putInt("musicImageId", music.getImageId());
